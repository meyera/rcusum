...
Cusum Control Chart Package - v0.7
========================================================
_Author_: Alex Meyer
...
To create a basic control charts 4 parameters have to be specified:
...
If the cusum curve...
 * ... lies __within the boundaries__, there is not enough evidence for a significant conclusion, thus there is still need for monitoring!
 * ... __crossed the upper boundary__, we can conclude that the failure rate has increased to the unacceptable event rate $p1$ ($p<0.05$)
 * ... __crossed the lower boundary__, we can conclude that the failure rate is less than or equal to the acceptable event rate $p0$ ($p<0.05$)
...
_"Interpretation of the graph in relation to the boundary lines is the same as for the cumulative failures chart. If performance is acceptable, the graph will tend downward toward the lower boundary; it will not follow the horizontal axis."_ (Rogers et. al, 2004)
...
By using the _Cumulative observed minus expected failure graph_, the graph will roughly oscillate around the horizontal axis if if the performance is does not deviate from the acceptable rate of failures. If any major trend is visible, most likely significant deviance is observed. 

This graph does __not show any inferential boundaries_, so it should be interpreted only in conjunction with the already mentioned graph types.
...
The failure rate is simulated using a probability of 10%
...
The upper graph is plotted again using a fixed $p0 = .10$ acceptable failure rate. The lower plot however accounts for a raising case complexity  using a simulated failure rate $p0i ~ N(µ=0.10, sd=0.03)$ for the first 100 patients, and then $p0i ~ N(µ=0.20, sd=0.03)$ each individual procedure.
...
#### Comparison of unadjusted und risk adjusted Cumulative Failure  and Cumulative log-likelihood ratio test charts